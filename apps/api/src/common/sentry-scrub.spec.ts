...
import { stripQuery, scrubRequest, scrubSpanAttributes } from './sentry-scrub.js';

describe('stripQuery', () => {
  it('쿼리스트링과 프래그먼트를 잘라낸다', () => {
    expect(stripQuery('/api/schedules?childId=abc')).toBe('/api/schedules');
    expect(stripQuery('https://x.dev/a/b?q=1#frag')).toBe('https://x.dev/a/b');
  });
...
describe('scrubRequest', () => {
  it('본문·쿠키·쿼리·헤더를 지우고 url은 남긴다', () => {
    const event = {
      request: {
        url: '/api/children?name=홍길동',
        method: 'POST',
        data: { name: '홍길동', memo: '조음 훈련 메모' },
        cookies: { access: 'jwt' },
        query_string: 'name=홍길동',
    ...
    expect(result.request).toEqual({ url: '/api/children', method: 'POST' });
  ...
  it('request가 없으면 그대로 반환한다', () => {
    const event = { transaction: 'GET /api/health' } as { request?: Record<string, unknown> };
...
describe('scrubSpanAttributes', () => {
  it('쿼리 전용 속성을 지운다', () => {
    const data = { 'url.query': 'note=SECRET', 'http.query': 'a=1', 'db.system': 'postgresql' };

    expect(scrubSpanAttributes(data)).toEqual({ 'db.system': 'postgresql' });
  });

  it('URL 속성에서 쿼리만 잘라낸다', () => {
    const data = {
      'http.url': 'https://api.eobom.dev/api/schedules?childId=abc',
      'http.target': '/api/schedules?childId=abc',
      'url.full': 'https://api.eobom.dev/x?note=SECRET',
    };

    expect(scrubSpanAttributes(data)).toEqual({
      'http.url': 'https://api.eobom.dev/api/schedules',
      'http.target': '/api/schedules',
      'url.full': 'https://api.eobom.dev/x',
    });
  });
