 ...
 *
 * `apps/web/src/shared/lib/sentry-scrub.ts`와 짝이다 — 패키지 경계 때문에 코드를
...
/**
 * Sentry의 `RequestEventData`에서 우리가 건드리는 부분만 추린 모양.
 * SDK 타입을 그대로 받으면 버전마다 필드가 늘어 결합이 생긴다.
 */
...
/**
 * 스팬/트랜잭션의 `data`(속성)에서 쿼리를 제거한다.
 * OpenTelemetry 계측이 채우는 키 이름이 SDK 버전에 따라 다르므로 둘 다 훑는다.
 */
